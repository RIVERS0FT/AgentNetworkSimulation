...
> 状态：目标设计与当前实现映射。本文定义剧本上传、下载、删除、预览和查询的需求边界、模块职责、接口、数据模型和迁移要求。SR 权威基线见 [系统需求规格.md](../requirements/系统需求规格.md)。
...
- Dashboard 剧本选择器按 ADR-033 使用 `scene_key` 作为稳定选项值、`title` 作为显示名称。
...
Dashboard 对查询结果的基础投影必须保持标识与显示名称分离：`scene_key` 用于配置、启动和资源寻址，`title` 只用于展示。不得重新读取旧的 `name`、`format` 字段，也不得使用可修改的 `title` 替代稳定标识。当前第一版列表只返回 `scene_key` 和 `title`，属于待继续扩展的实现状态，不改变本节面向 `SR-SCENE-05` 的目标字段集合。
