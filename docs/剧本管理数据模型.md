...
- **类**：具有独立标识、生命周期或业务行为的领域对象，类图中同时列出字段和领域函数；
- **结构体**：只承载定义、请求或结果数据，不定义有副作用的业务函数；
- **枚举**：表达有限且稳定的状态和值域；
- 当前阶段以文件目录扫描和按需解析实现，不引入数据库表；
- `storage_locator`、`module_locator` 等定位信息仅供内部模块使用，不直接向 Web 前端暴露物理路径；
- 剧本查询不提供排序能力，只支持过滤和分页；
- 当前并发仿真上限为 `1`，但剧本占用状态使用运行引用集合表达，未来可以扩展为多个并行仿真；
- Skill 与 Tool 是不同种类的资源，必须在 `SceneDefinition` 中分别存储和校验，不使用统一的能力定义集合。
...
`SceneDefinition` 是剧本解析后的完整内存定义。Skill 与 Tool 分开保存：
...
| `skills` | `SkillDefinition[]` | 是 | 当前剧本 `skills` 资源扫描和解析结果。 |
...
- `AgentDefinition.skill_refs` 只能引用 `SceneDefinition.skills` 中存在的 `skill_ref`；
- `AgentDefinition.allowed_tools` 只能引用 `SceneDefinition.tools` 中存在的 `tool_ref`；
- Skill 的源文件读取权限与 Tool 的执行权限必须分别校验。
...
| `entrypoint` | string | 是 | Skill 入口资源，如包内 `SKILL.md` 或单文件入口。 |
...
| `network_mode` | string | 否 | 当前为 `direct`。 |
...
当前代码中的 `SceneDefinition` 只直接保存 Agent 和拓扑；Skill 引用和 Tool 授权目前保存在每个 `AgentDef` 中。本文确定目标模型应在解析剧本时分别构造 `skills` 和 `tools` 定义集合，再由 Agent 保存引用。

该调整不要求把 Skill 当作 Tool，也不允许用统一 `CapabilityReference` 混合表示两者。实现时必须同步修改解析器、校验、预览返回模型和测试。